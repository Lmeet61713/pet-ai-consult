...
if [[ -z "$SUPERVISORD_BIN" ]]; then
    echo "supervisord is required to run the Vision service" >&2
    exit 1
fi

mkdir -p "$PROJECT_ROOT/runtime/vision"
export PET_CONSULT_ROOT="$PROJECT_ROOT"
exec "$SUPERVISORD_BIN" -c "$PROJECT_ROOT/configs/vision-supervisord.conf"
